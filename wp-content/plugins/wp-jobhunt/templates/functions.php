    ...
    function cs_pagination($total_records, $per_page, $qrystr = '', $show_pagination = 'Show Pagination', $query_string_variable = 'page_id_all') {
        ...
        if ( $show_pagination <> 'Show Pagination' ) {
            ...
            if ( $page_id_all > 1 ) {
                ...
                $html .= "<li><a href='?$query_string_variable=" . ($page_id_all - 1) . "$qrystr' aria-label='" . esc_html__('Previous', 'jobhunt') . "' ><span aria-hidden='true'><i class='icon-angle-left'></i> " . esc_html__('Previous', 'jobhunt') . " </span></a></li>";
            } else {
            ...
            if ( $per_page > 0 and $page_id_all < $total_records / $per_page ) {
                $html .= "<li><a aria-label='" . esc_html__('Next', 'jobhunt') . "' href='?$query_string_variable=" . ($page_id_all + 1) . "$qrystr' ><span aria-hidden='true'>" . esc_html__('Next', 'jobhunt') . " <i class='icon-angle-right'></i></span></a></li>";
            } else {
            ...
            if ( (isset($_POST['post_id']) && $_POST['post_id'] <> '' ) ) {
                ...
                if ( $response['status'] == 1 ) {
                    ...
                    if ( isset($cs_wishlist) && in_array($_POST['post_id'], $cs_wishlist) ) {
                        ...
                        $response ['status'] = 1;
                        $response ['msg'] = '<i class="icon-thumbsup"></i><span>' . esc_html__('Applied', 'jobhunt') . '</span>';
                    }
                    $cs_wishlist = array();
...
if ( ! function_exists('cs_get_job_specialisms_dropdown') ) {
    ...
    function cs_get_job_specialisms_dropdown($name, $id, $selected_post_id = '', $class = '', $required_status = 'false') {
        ...
        $cs_opt_array = array(
            ...
            'std' => $selected_slug,
            'desc' => '',
            'extra_atr' => 'data-placeholder="' . esc_html__("Please Select specialism", "jobhunt") . '"',
            'classes' => $class,
            'options' => $select_options,
            'hint_text' => '',
            'required' => 'yes',
...
if ( ! function_exists('cs_get_specialisms_dropdown') ) {
    ...
    function cs_get_specialisms_dropdown($name, $id, $user_id = '', $class = '', $required_status = 'false') {
        ...
        if ( ! empty($terms) ) {
            ...
            $output .= esc_html__('There are no specialisms available.', 'jobhunt');
        }
        return $output;
    }
